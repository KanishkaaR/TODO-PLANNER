...
class BaseComponent {
  varructor(element) {
    ...
    Data.set(this._element, this.varructor.DATA_KEY, this);
  }

  dispose() {
    Data.remove(this._element, this.varructor.DATA_KEY);
    EventHandler.off(this._element, this.varructor.EVENT_KEY);
...
class Alert extends BaseComponent {
  ...
  static jQueryInterface(config) {
    return this.each(function () {
      ...
      if (data[config] === undefined || config.startsWith('_') || config === 'varructor') {
...
var SelectorEngine = {
  ...
  focusableChildren(element) {
    var focusables = ['a', 'button', 'input', 'textarea', 'select', 'details', '[tabindex]', '[contenteditable="true"]'].map(selector => `${selector}:not([tabindex^="-"])`).join(', ');
...
class Carousel extends BaseComponent {
  ...
  static dataApiClickHandler(event) {
    ...
    var config = { ...Manipulator.getDataAttributes(target),
...
class Dropdown extends BaseComponent {
  ...
  _getConfig(config) {
    config = { ...this.varructor.Default,
      ...Manipulator.getDataAttributes(this._element),
      ...config
    };
    typeCheckConfig(NAME$9, config, this.varructor.DefaultType);
...
class Tooltip extends BaseComponent {
  ...
  show() {
    ...
    var showEvent = EventHandler.trigger(this._element, this.varructor.Event.SHOW);
    ...
    if (this.varructor.NAME === 'tooltip' && this.tip && this.getTitle() !== this.tip.querySelector(SELECTOR_TOOLTIP_INNER).innerHTML) {
      this._disposePopper();
    ...
    var tipId = getUID(this.varructor.NAME);
    ...
    Data.set(tip, this.varructor.DATA_KEY, this);

    if (!this._element.ownerDocument.documentElement.contains(this.tip)) {
      container.append(tip);
      EventHandler.trigger(this._element, this.varructor.Event.INSERTED);
    }
    ...
    var complete = () => {
      ...
      EventHandler.trigger(this._element, this.varructor.Event.SHOWN);
  ...
  hide() {
    ...
    var complete = () => {
      ...
      EventHandler.trigger(this._element, this.varructor.Event.HIDDEN);

      this._disposePopper();
    };

    var hideEvent = EventHandler.trigger(this._element, this.varructor.Event.HIDE);
  ...
  _initializeOnDelegatedTarget(event, context) {
    return context || this.varructor.getOrCreateInstance(event.delegateTarget, this._getDelegateConfig());
  }
  ...
  _getPopperConfig(attachment) {
    var defaultBsPopperConfig = {
      ...
      modifiers: [{
        ...
        name: 'arrow',
        options: {
          element: `.${this.varructor.NAME}-arrow`
        }
      }, {
        name: 'onChange',
        enabled: true,
  ...
  _setListeners() {
    ...
    triggers.forEach(trigger => {
      if (trigger === 'click') {
        EventHandler.on(this._element, this.varructor.Event.CLICK, this._config.selector, event => this.toggle(event));
      } else if (trigger !== TRIGGER_MANUAL) {
        var eventIn = trigger === TRIGGER_HOVER ? this.varructor.Event.MOUSEENTER : this.varructor.Event.FOCUSIN;
        var eventOut = trigger === TRIGGER_HOVER ? this.varructor.Event.MOUSELEAVE : this.varructor.Event.FOCUSOUT;
  ...
  _getConfig(config) {
    ...
    typeCheckConfig(NAME$4, config, this.varructor.DefaultType);
  ...
  _getDelegateConfig() {
    ...
    for (var key in this._config) {
      if (this.varructor.Default[key] !== this._config[key]) {
        config[key] = this._config[key];
      }
    } // In the future can be replaced with:
    // var keysWithDifferentValues = Object.entries(this._config).filter(entry => this.varructor.Default[entry[0]] !== this._config[entry[0]])
...
var DefaultType$2 = { ...Tooltip.DefaultType,
...
class ScrollSpy extends BaseComponent {
  ...
  _activate(target) {
    ...
    var queries = SELECTOR_LINK_ITEMS.split(',').map(selector => `${selector}[data-bs-target="${target}"],${selector}[href="${target}"]`);
...
class Toast extends BaseComponent {
  ...
  _getConfig(config) {
    ...
    typeCheckConfig(NAME, config, this.varructor.DefaultType);
    return config;
  }
